...
function () {
  function Moon(jde) {
    ...
    var F = this.F = base.horner(T, 93.272095 * D2R, 483202.0175233 * D2R, -0.0036539 * D2R, -D2R / 3526000, D2R / 863310000);
    this.Ω = base.horner(T, 125.0445479 * D2R, -1934.1362891 * D2R, 0.0020754 * D2R, D2R / 467441, -D2R / 60616000); // true ecliptic
  ...
  _proto.optical = function optical(λ, β) {
    // (53.1) p. 372
  ...
  _proto.physical = function physical(A, b_) {
    // (53.2) p. 373
    var _base$sincos4 = base.sincos(A),
        sA = _base$sincos4[0],
        cA = _base$sincos4[1];
  ...
  _proto.pa = function pa(λ, β, b) {
    var V = this.Ω + this.Δψ + this.σ / sI;

    var _base$sincos5 = base.sincos(V),
        sV = _base$sincos5[0],
        cV = _base$sincos5[1];

    var _base$sincos6 = base.sincos(_I + this.ρ),
        sIρ = _base$sincos6[0],
        cIρ = _base$sincos6[1];
  ...
  _proto.sun = function sun(λ, β, Δ, earth) {
...
export function sunAltitude(cOnMoon, cSun) {
  // (η, θ, l0, b0 float64)  float64
  var c0 = Math.PI / 2 - cSun.lon;

  var _base$sincos7 = base.sincos(cSun.lat),
      sb0 = _base$sincos7[0],
      cb0 = _base$sincos7[1];

  var _base$sincos8 = base.sincos(cOnMoon.lat),
      sθ = _base$sincos8[0],
      cθ = _base$sincos8[1];
...
export function sunrise(cOnMoon, jde, earth) {
...
export function sunset(cOnMoon, jde, earth) {
  // (η, θ, jde float64, earth *pp.V87Planet)  float64
  jde += srCorr(cOnMoon, jde, earth);
  return jde + srCorr(cOnMoon, jde, earth);
}
...
var lunarCoord = function lunarCoord(η, θ) {
  return new base.Coord(η * D2R, θ * D2R);
};
/**
